"""
ds001907_glm_aparc.py
---------------------
Même logique que oasis1_glm_aparc.py, adapté pour ds001907 (Parkinson + sujets âgés sains).
Pour chaque région APARC (34 lh + 34 rh) :
    base   : thickness ~ age + sex
    motion : thickness ~ age + sex + motion
Calcule ΔAIC, coefficients, sélectionne ~25 sujets stratifiés pour JDAC.

Usage (sur le PC du labo) :
    python ds001907_glm_aparc.py \
        --scores       ~/Documents/motion-analysis/datasets/ds001907/results/ds001907_scores.csv \
        ...
        --out_dir      ~/Documents/motion-analysis/datasets/ds001907/results

Notes :
    - ds001907 : Attention Network Test, Parkinson + sujets âgés sains
    - Colonne group dans participants.tsv : "patient" / "control"
    - Le GLM inclut group comme covariable supplémentaire
    - Si FreeSurfer non dispo dans les dérivés, adapter --fs_dir
"""
...
# ---------------------------------------------------------------------------
# Lecture aparc.stats (identique à oasis1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GLM par région — inclut group (Parkinson vs contrôle)
# ---------------------------------------------------------------------------

def fit_region(df, col, has_group=True):
    covars = ["age", "sex_num", "motion"]
    if has_group:
        covars.append("group_num")
    sub = df[covars + [col]].dropna()
    if len(sub) < 20:
        return None

    y  = sub[col]
    Xb = sm.add_constant(sub[["age", "sex_num"] + (["group_num"] if has_group else [])])
    Xm = sm.add_constant(sub[covars])
...
def plot_delta_aic(df, out_path, title_suffix=""):
...
def main():
    ...
    parser.add_argument("--out_dir",      default="./ds001907_results")
    ...
    # participant_id → sub (format BIDS standard : sub-XXXX)
    if "participant_id" in demo.columns:
        demo["sub"] = demo["participant_id"].astype(str)
    else:
        raise ValueError("Colonne participant_id introuvable")

    sex_col = next((c for c in demo.columns if c.lower() in ["sex", "gender"]), None)
    if sex_col is None:
        raise ValueError("Colonne sex/gender introuvable")
    ...
    if age_col is None:
        raise ValueError("Colonne age introuvable")
    demo["age"] = demo[age_col]

    # Group : Parkinson vs contrôle
    group_col = next((c for c in demo.columns if "group" in c.lower() or "diagnosis" in c.lower()), None)
    has_group = group_col is not None
    if has_group:
        demo["group"]     = demo[group_col]
        demo["group_num"] = (demo[group_col].astype(str).str.lower().str.contains("park|patient")).astype(int)
        print(f"  Colonne groupe trouvée : '{group_col}'")
        print(f"  Patients : {demo['group_num'].sum()} | Contrôles : {(demo['group_num']==0).sum()}")
    else:
        print("  Aucune colonne groupe trouvée — GLM sans covariable group")

    keep_cols = ["sub", "age", "sex", "sex_num"] + (["group", "group_num"] if has_group else [])
    merged = scores.merge(demo[keep_cols], on="sub", how="inner")
    ...
    res_csv = out / "ds001907_glm_aparc.csv"
    ...
    plot_delta_aic(res_df, out / "ds001907_aic_delta.png")

    print("\nSélection des sujets pour JDAC...")
    select_subjects(full, args.bids_root, out / "ds001907_jdac_subjects.csv")
